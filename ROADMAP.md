...
1. **TOEIC予測**: 回帰式(係数)を後日受領。それまで結果画面は「準備中(Coming soon)」表示。係数は `data/conversion.js` に分離し、timed/untimed 別の係数枠を用意して差し替え可能にする。
...
- [x] **参加者向けUIの日本語化**(日本語デフォルト+ENトグル)。ボタンは「適切/Yes」「不適切/No」併記。研究者セットアップ画面は英語のまま
...
- [x] **結果画面の刷新**: 素点(x/40・正答率)を即時表示。TOEIC Listening予測とCEFR参考レベルの表示枠を実装し、係数受領まで「準備中」表示。CEFRはETS公式カットスコア(L: A1=60 / A2=110 / B1=275 / B2=400 / C1=490)を用い、「公式スコアではなく参考推定値」の免責文言を必ず併記
...
| **M3: 一般公開+掲載** | 5, 8, 9 | **倫理承認の確認**、TOEIC回帰式は後追い差し替え可 |

## 外部依存(ブロッカーになり得るもの)

1. **TOEIC回帰式の受領**(小室先生側)— M3を止めない(「準備中」表示で公開可)。受領後 `data/conversion.js` 差し替え+免責文言レビューのみ
